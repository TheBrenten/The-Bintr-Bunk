...
vec screenShakePos;
vec screenShakePosInterp  ;
float screenShakeAngInterp = 0;

void physFunc()
{
    // screen shake
    {
        screenShakePosInterp = screenShakePos;
        screenShakeAngInterp = screenShakeAngle;
    ...
    //// animate uh
    //// for all active animations (recoil/reload)...
    //for (int i = 0; i < activeKeyFrames.size(); i++) {
    //    KeyFrame* kf = activeKeyFrames[i];
    //    // for every object involved in animation...
    //    for (int j = 0; j < kf->displacements.size(); j++) {
    //        Displacement* d = &kf->displacements[j];
    //        // if the altered fuck
    //        if (d->objP && d->relativeObjP) {
    //
    //        }
    //    }
    //}
    ...
    for (int i = activeSounds.size() - 1; i >= 0; i--) {
        if (activeSounds[i]->sound.getStatus() != Sound::Playing) {
            ...
            //activeSounds[i].worldNozzle->print();
