...
void drawHotbar(vec hbPos, float hbRad) {
    ...
    if ((selector - hbPos).getLength() < hbRadInner) {
        ...
        if (hotbarItems[slot]->isWeapon) {
            if (justPressedLeftClick) { p->setLeftItem(hotbarItems[slot]->weaponP);}// p->leftHanded = true; }
...
void renderFunc()
{
    ...
    // input events
    {
        ...
        if (windowFocused) {
            ...
            if (sf::Keyboard::isKeyPressed(Keyboard::X)) hotbar = true;
            if (sf::Keyboard::isKeyPressed(Keyboard::E)) inventory = true;
        ...
        while (window->pollEvent(event))
        {
            ...
            if (event.type == sf::Event::KeyPressed) {
                ...
                if (event.key.code == sf::Keyboard::P) {
                    if (physRate == 60) physRate = 15;
                    else if (physRate == 15) physRate = 240;
                    else if (physRate == 240) physRate = 60;
                }

                //if (event.key.code == sf::Keyboard::W) if (activeMenuObj != NULL && activeMenuObj->neighborU) { activeMenuObj->pressed = false; activeMenuObj = activeMenuObj->neighborU; }
                ...
                if (event.key.code == sf::Keyboard::T) noclip = !noclip;
    ...
    camView1.move(screenShakePos.x, screenShakePos.y);
    camView1.setRotation(screenShakeAngle);
    ...
    for (int i = renderObjects.size() - 1; i >= 0; i--) {
        ...
        obj->posInterp = obj->posInterpLast + (obj->pos - obj->posInterpLast) * interp;
        obj->renderBack();
