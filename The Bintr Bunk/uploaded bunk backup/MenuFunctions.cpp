...
void respawnPlayer() {
    ...
    if (ppLShoulder) delete ppLShoulder;
    if (ppLHand) delete ppLHand;
    if (lArmSpring) delete lArmSpring;
    if (lArmStick) delete lArmStick;
    if (ppRShoulder) delete ppRShoulder;
    if (ppRHand) delete ppRHand;
    if (rArmSpring) delete rArmSpring;
    if (rArmStick) delete rArmStick;
    //pp = &p->point;
    float strength = 0.1;
    handGravity = 0.5;
    float fric = 0.9;
    int springDist = 30;
    int maxDist = 62;
    ppLShoulder = new PhysPoint();
    ppLHand = new PhysPoint; ppLHand->gravity = handGravity; ppLHand->friction = fric;
    lArmSpring = new Spring(ppLShoulder, ppLHand, strength, 0, springDist);
    lArmStick = new Stick(ppLShoulder, ppLHand, 0, maxDist);
    ppRShoulder = new PhysPoint();
    ppRHand = new PhysPoint; ppRHand->gravity = handGravity; ppRHand->friction = fric;
    rArmSpring = new Spring(ppRShoulder, ppRHand, strength, 0, springDist);
    rArmStick = new Stick(ppRShoulder, ppRHand, 0, maxDist);
