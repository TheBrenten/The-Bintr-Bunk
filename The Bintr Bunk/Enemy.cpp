...
void Enemy::move() {
    ...
    if (p) {
        ...
        if (health <= 0) {
            ...
            soundPitch *= randnum(0.993, 0.998);
            ...
            //new Dot(goalPos);
            for (int i = 0; i < p->body.inRegions.size(); i++) {
                for (auto it = p->body.inRegions[i]->nodes.begin(); it != p->body.inRegions[i]->nodes.end(); it++) {
                    Node* n = it->second;
                    if (n->validNeighbors.size() > 0) {
...
void Enemy::render() {
    ...
    vec pos2 = interpVec(posInterp, pos);
    float ang2 = interpAng(angInterp, angle);
    sprite.setPosition(pos2.x, pos2.y);
    sprite.setRotation(ang2);
    ////disp_circ.setFillColor(Color(0,0,254));
    //disp_circ.setPosition(worldNozzle.x,worldNozzle.y);
